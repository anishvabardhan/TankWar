...
{
	LogHook(LoggerHookHandler); // Hook into the logger to print on console
	...
	m_selectionColor = Rgba(100,200,0,150);
...
void DevConsole::Render() const
{
	...
	renderer.DrawAABB2(GetConsoleBounds(), Rgba(0,0,100,50));
	renderer.DrawAABB2(GetTextWindowBounds(), Rgba(100,100,100,50));
	renderer.DrawAABB2(GetInputBounds(), Rgba(0,0,0,50));
...
void DevConsole::RenderConsoleText(Renderer& renderer) const
{
	...
	// Thread Safety must be manually ensured
	m_consoleTextBuffer.LockEnter();
	for(auto line = m_consoleTextBuffer.rbegin() + m_scrollIndex; line != m_consoleTextBuffer.rend() && lineIndex < m_numLinesToDisplay; ++line)
	{
...
{
	m_consoleTextBuffer.PushBack(ColoredText(text,color));
}
...
//-----------------------------------------------------------------------------------------------
// Destroys the dev console instance
//
void DevConsole::DestroyInstance()
{
	if(g_theConsole)
	{
		delete g_theConsole;
...
//-----------------------------------------------------------------------------------------------
// Handles the logger hooks 
//
void DevConsole::LoggerHookHandler(const LogMessage& msg, void* userData)
{
	ConsolePrintf("%s: %s", msg.tag.c_str(), msg.text.c_str());
}
...
void ConsoleStartup()
{
	#if defined(ENGINE_ENABLE_CONSOLE)
		DevConsole::CreateInstance(Window::GetInstance()->m_width, Window::GetInstance()->m_height);
		DevConsole::GetInstance()->SetFont(*Renderer::GetInstance()->CreateOrGetBitmapFont("SquirrelFixedFont")); // Sets console font
...
{
	#if defined(ENGINE_ENABLE_CONSOLE)
		CommandDefinition::CommandShutdown();
		DevConsole::DestroyInstance();
	#endif
}
