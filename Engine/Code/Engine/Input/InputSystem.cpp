...
//-----------------------------------------------------------------------------------------------
// Starts up the input system
//
void InputSystemStartup()
{
	#if defined(ENGINE_ENABLE_INPUT)
...
//-----------------------------------------------------------------------------------------------
// Returns the Mouse position in GL coords (Y flip)
//
Vector2 InputSystem::GetMousePosition()
{
	Vector2 clientPos = g_theInput->mouse.GetMouseClientPosition();
	
	Vector2 glPos = clientPos;
	glPos.y = Window::GetInstance()->m_height - clientPos.y;
