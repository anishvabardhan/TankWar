...
void main( void )
{
   ...
   vec2 uv_offset = passUV + vec2(GAME_TIME * .1f); // Tiling water
   vec4 texColor = texture( gTexDiffuse, passUV + uv_offset );
   ...
   final_color *= final_color.a; // multiply alpha through
}